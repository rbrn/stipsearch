package com.db.pwcc.tre.elastic.web.rest;

import com.db.pwcc.tre.elastic.config.KafkaProperties;
...
class StipsearchKafkaResourceIT {
    ...
    void setup() {
        ...
        StipsearchKafkaResource kafkaResource = new StipsearchKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/stipsearch-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/stipsearch-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
